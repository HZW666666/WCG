...
void vLEDShark(void *pvParameters) {
    ...
    //    char* puBuffer = uBuffer;
    for (;;) {
        _LATE7 ^= 1; //green led
        vTaskDelay(500);
        //        sprintf(puBuffer, "xNowWakeTime:\r\n"); //, xNowWakeTime);
        //        puBudffer += strlen(puBuffer);
    }
}

void vTimeUntilTask(void *parameter) {
    TickType_t xPreviousWakeTime;
    TickType_t xNowWakeTime;
    ...
    for (;;) {
        _LATE5 ^= 1;
        xNowWakeTime = xTaskGetTickCount();
        Uart3SendString("xNowWakeTime:\t");
        Uart3SendInterger(xNowWakeTime);
        Uart3SendString("\r\n");
        vTaskDelayUntil(&xPreviousWakeTime, 3000);//pdMS_TO_TICKS(50));
    }
}

void task_start(void *parameter) {
    xTaskCreate(vLEDShark, "vLEDShark", mainCHECK_TAKS_STACK_SIZE, NULL, mainCHECK_TASK_PRIORITY, NULL);
    xTaskCreate(vTimeUntilTask, "vStatusTask", mainCHECK_TAKS_STACK_SIZE, NULL, vTaskStatu_PRIORITY, xStateTask);
    vTaskDelete(NULL);
}
